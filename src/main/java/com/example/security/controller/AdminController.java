...
public class AdminController {
    ...
    public ResponseEntity<String> updateCake(@RequestBody Cake cake) {
        try {
            String result = cakeService.updateCake(cake);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping("/chocolate/{name}")
    public ResponseEntity<String> deleteChocolate( @PathVariable String name) {
        try {
            String result = chocolateService.deleteChocolate(name);
    ...
    public ResponseEntity<String> updateChocolate(@RequestBody Chocolate chocolate) {
        try {
            String result = chocolateService.updateChocolate(chocolate);
            if (result.contains("updated")) {
                return new ResponseEntity<>(result, HttpStatus.OK);
            }
            return new ResponseEntity<>(result, HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<List<Chocolate>> getAllChocolates() {
        try {
            return ResponseEntity.ok().body(chocolateService.getAllChocolates());
        } catch (Exception e) {
            return new ResponseEntity(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
